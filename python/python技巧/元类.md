...
class的定义是运行时动态创建的，创建class的方法就是使用`type()`函数。
...
Student = type('Student', (object, ), dict(__init__=__init__, hello=hello))
s = Student('lily')
s.hello()
```
...
```
type(object_or_name, bases, dict) -> the object's type
```

对应上述创建方法：

- `object_or_name`: `'Student'`类名，可以用`Student.__name__`查看
- `bases`: 继承的父类集合
...
- 核心是创建了两个属性`__mappings__`和`__table__`
- 这样实例化后的对象就多了这两个属性
...
> 实现了`__getattr__`和`__setattr__`后就可以直接`u.name`调用属性了。
